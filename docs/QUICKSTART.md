...
npm install
REDIS_URL=redis://localhost:6379 POW_SERVICE_URL=http://localhost:3001 npm run start:coordinator

# Run worker locally
WORKER_ID=dev-worker-1 REDIS_URL=redis://localhost:6379 POW_SERVICE_URL=http://localhost:3001 npm run start:worker
```
...
npm test -- scripts/tests/test-batch-queue.js
npm test -- scripts/tests/test-worker-integration.js
```
